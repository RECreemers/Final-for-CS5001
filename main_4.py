...
class Snake():
    ...
    def draw_snake(self):
        """
        This section allows the snake to be drawn onscreen, and creates the conditions for what image
        is displayed depending on where the snake boxes are in relation to eachother.
        """
    ...
    def move_snake(self):
        """
        This section allows the snake to move forward by copying the snake_body list to modify it, adding
        a new Vector2 to the list, and removing the last Vector2 from the list. If the snake touches a 
        fruit, the add_snake_bock is set to True and the last Vector 2 is simply not removed
    ...
    def add_block(self):
        """
        sets add_snake_block to True so that move_snake knows it's got to add a block
        """
        self.add_snake_block = True

    def update_head_graphics(self):
        """
        This is where the first Vector2 in the list of the snake_body checks to see what direction it
        is facing and thusly, which snake_head image should be on screen. It does this by looking at 
        the first block and subracts the second block vectors from it. Since everything has been made
        in chuncks it allows for the clean number check of -1 or +1 in either the x or y of the Vector2
        list. 
        """
    ...
    def update_butt_graphics(self):
        """
        This is where the last Vector2 in the list of the snake_body checks to see what direction it
        is facing and thusly, which snake_head image should be on screen. It does this by looking at the
    ...
    def reset_snake(self):
        """
        This allows the snake to restart at the same place with the same size list as the beginning
        """
...
class Fruit():
    ...
    def randomize(self):
        """
        takes a random x,y coordinate on the screen and moves the fruit to that spot. It also selects a
        fruit randomly, either the golden or red apple
        """
...
class Main():
    """
    This is where the looping part of the program runs, the part that has to be executed every
    time the screen refreshes and the logic behind the game 
    """
    ...
    def updates(self):
        """
        The basic update area, checking to see where the snake moves, if it got a fruit, or if you died,
        as well as keeping the timer going for the music
        """
    ...
    def draw_elements(self):
        """
        Because the snake moves every second of the game, the draw elements need to be continually updated
        /refreshed.
        """
    ...
    def check_collision(self):
        """
        This portion checks if the snake touched the apple(depending on which one) the music changes, the
        apple and it's position are randomized, the snake gets a block, and the score is increased
        """
    ...
    def check_fail(self):
        """
        setting ways for the game to end, such as snake touches tail or snake tries to go outside
        the screen
        """
    ...
    def draw_score(self):
        """
        The score needs to be reactive, so the numbers and letters have to be drawn on screen
        with pygame as opposed to using a jpg.
        """
...
class Popups():
    ...
    def __init__(self) -> None:
        self.snake = Snake()
        self.main = Main()
        self.font_big = pygame.font.Font("Art/Next_Sunday.ttf", 80)
        self.font_small = pygame.font.Font("Art/Next_Sunday.ttf", 60)
        self.title = self.font_big.render("Snake Game", True, (255, 255, 255))
        self.start_button = self.font_small.render("Start", True, (color_start))
        self.quit_button = self.font_small.render("Quit", True, (color_quit))
        self.restart_button = self.font_small.render("Restart", True, (color_restart))
    ...
    def draw_title_popup(self):
        ...
        #Setting Positions
        pos_title_rect = title_rect.get_rect(center = (screen_center, screen_center))
        pos_title = self.title.get_rect(center = (screen_center, (screen_center / 1.25)))
        pos_start_button = self.start_button.get_rect(center = (((screen_center) / 1.5), (screen_center + 75)))
        pos_restart_button = self.restart_button.get_rect(center = (((screen_center) / 1.5), (screen_center + 75)))
        pos_quit_button = self.quit_button.get_rect(center = ((screen_center + ((screen_center) / 2.75)), (screen_center + 75)))
        screen.fill((122, 193, 122))
        #Write on Text
        if self.snake.snake_alive == True:  # If this is the first round through
